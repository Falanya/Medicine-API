    ...
    <div class="col-md-7 col-md-offset-1">
        ...
        <div class="panel panel-info">
            <div class="panel-body">
                <form action="{{ route('account.check_edit_address', $address->id) }}" method="POST" role="form">
                    @csrf
                    <legend>Edit address</legend>

                    <div class="form-group">
                        <label for="">Receiver Name</label>
                        <input type="text" value="{{ $address->receiver_name }}" class="form-control" name="receiver_name" placeholder="Input receiver name">
                        @error('receiver_name') <small> {{ $message }} </small> @enderror
                    </div>

                    <div class="form-group">
                        <label for="">Address</label>
                        <input type="text" value="{{ $address->address }}" class="form-control" name="address" placeholder="Input address">
                        @error('address') <small> {{ $message }} </small> @enderror
                    </div>

                    <div class="form-group">
                        <label for="">Phone</label>
                        <input type="text" value="{{ $address->phone }}" class="form-control" name="phone" placeholder="Input phone">
